...
public class DocumentApiTests : IClassFixture<ApiTestFixture>
{
    ...
    {
        // Arrange
        var payload = TestDataFactory.Document();

        // Act
        var response = await _client.PostAsync("/api/Document", JsonSnakeCaseContent.From(payload));
    ...
    public async Task GetById_WhenUnknown_ReturnsNotFound()
    {
        ...
        // Act
        var response = await _client.GetAsync("/api/Document/" + id);
    ...
    public async Task Put_WhenIdMismatch_ReturnsBadRequestOrUnprocessable()
    {
        ...
        // Act
        var response = await _client.PutAsync("/api/Document/" + id, JsonSnakeCaseContent.From(payload));
