...
public class PatientApiTests : IClassFixture<ApiTestFixture>
{
    ...
    {
        // Arrange
        var payload = TestDataFactory.Patient();

        // Act
        var response = await _client.PostAsync("/api/Patient", JsonSnakeCaseContent.From(payload));
    ...
    public async Task GetById_WhenUnknown_ReturnsNotFound()
    {
        ...
        // Act
        var response = await _client.GetAsync("/api/Patient/" + id);
    ...
    public async Task Put_WhenIdMismatch_ReturnsBadRequestOrUnprocessable()
    {
        ...
        // Act
        var response = await _client.PutAsync("/api/Patient/" + id, JsonSnakeCaseContent.From(payload));
    ...
    public async Task Delete_WhenUnknownId_ReturnsNotFoundOrNoContent()
    {
        ...
        // Act
        var response = await _client.DeleteAsync("/api/Patient/" + id);
