...
public class StaffApiTests : IClassFixture<ApiTestFixture>
{
    ...
    {
        // Arrange
        var payload = TestDataFactory.Staff();

        // Act
        var response = await _client.PostAsync("/api/Staff", JsonSnakeCaseContent.From(payload));
    ...
    public async Task GetById_WhenUnknown_ReturnsNotFound()
    {
        ...
        // Act
        var response = await _client.GetAsync("/api/Staff/" + id);
    ...
    public async Task Put_WhenIdMismatch_ReturnsBadRequestOrUnprocessable()
    {
        ...
        // Act
        var response = await _client.PutAsync("/api/Staff/" + id, JsonSnakeCaseContent.From(payload));
    ...
    public async Task Delete_WhenUnknownId_ReturnsNotFoundOrNoContent()
    {
        ...
        // Act
        var response = await _client.DeleteAsync("/api/Staff/" + id);
